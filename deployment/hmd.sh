...
if [[ ! $BRANCH ]]; then
	log "Branch not specified in env vars"
	die
fi
...
while [ -f /root/hmd.lock ]; do
	...
	if [ $(( CURR_TIME - START_TIME )) -gt 480 ]; then
		log "Timeout occured while waiting for the process to obtain the lock file"
		die
	fi
done
...
npm --prefix $REPO_LOC_SERVER install $REPO_LOC_SERVER && log "Installing node server dependencies"
if [ -e "$REPO_LOC_CLIENT/package.json" ]; then
	npm --prefix $REPO_LOC_CLIENT run build $REPO_LOC_CLIENT && log "Installing node client dependencies"
fi
# Start pm2
